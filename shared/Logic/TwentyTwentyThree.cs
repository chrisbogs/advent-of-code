...
namespace AdventOfCodeShared.Logic
{
    public class TwentyTwentyThree
    {
        ...
        public static long Day5Part1(string[] input)
        {
            ...
            // Parse input into map objects

            var seeds1 = input[0].Split("seeds:");
            List<int> seedNumbers = seeds1[1].Trim().ParseIntsOnLine(" ");
            List<int> locationNumbers = new List<int>(seedNumbers.Count);

            List<List<MappingRange>> mappings = new List<List<MappingRange>>()
            {
            new(),//List<MappingRange> seedToSoil = new();
            new(),//List<MappingRange> soilToFertilzer = new();
            new(),//List<MappingRange> fertilizerToWater = new();
            new(),//List<MappingRange> waterToLight = new();
            new(),//List<MappingRange> lightToTemp = new();
            new(),//List<MappingRange> tempToHumidity = new();
            new(),//List<MappingRange> humidityToLocation = new();
            };
            ...
            for (int i = 1; i < input.Length; i++)
            {
                ...
                {
                    var nums = line.ParseIntsOnLine(" ");
                    mappings[currentMapIndex].Add(new MappingRange(nums[0], nums[1], nums[2]));
                }
            }

            for(int i = 0; i < seedNumbers.Count;i++)
            {
                var currentNumber = seedNumbers[i];
                foreach(var mapping in mappings)
                {
                    int? mappedNumber = null;
                    // find a range that the number fits into
                    foreach(var mappingRange in mapping)
                    {
                        if (mappingRange.IsInSourceRange(currentNumber))
                        {
                            mappedNumber = mappingRange.GetDestination(currentNumber);
                        }
                    }

                    if (mappedNumber is null)
                    {
                        mappedNumber = currentNumber;// means it maps to same number
                    }

                    currentNumber = mappedNumber.Value;
                }
        ...
        public class MappingRange
        {
            ...
            public MappingRange(int d, int s, int r)
            {
                DestinationStartRange = d;
                SourceStartRange = s;
                RangeLength = r;
            }
            public bool IsInSourceRange(int n)
            {
                return n >= SourceStartRange && n <= SourceStartRange + RangeLength;
            }
            public int? GetDestination(int n)
            {
                if (IsInSourceRange(n))
                {
                    var sourceIndex = n - SourceStartRange;
                    return DestinationStartRange + sourceIndex;
                }
                return null;
            }
        ...
        public static long Day6Part1(string[] input)
        {
            ...
            //As you try to figure out what to do next, you notice a poster on a wall near the ferry dock. "Boat races! Open to the public! Grand prize is an all-expenses-paid trip to Desert Island!"
            ...
            //The organizer explains that it's not really a traditional race - instead, you will get a fixed amount of time during which your boat has to travel as far as it can,
            ...
            //Hold the button for 7 milliseconds.That's the entire duration of the race. You never let go of the button. The boat can't move until you let go of the button.
            ...
            var times = input[0].ParseIntsOnLine(" ");
            var recordDistances = input[1].ParseIntsOnLine(" ");
